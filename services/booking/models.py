...
class Booking:
    @staticmethod
    def create(cursor, user_id: int, room_id: int, check_in_date: date, 
        ...
        query = """
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """
        cursor.execute(query, (user_id, room_id, check_in_date, check_out_date,
                             total_price, special_requests))
        cursor.execute("SELECT LAST_INSERT_ID()")
    ...
    def cancel_booking(cursor, booking_id: int) -> Optional[Dict[str, Any]]:
        ...
        if not booking or booking["status"] in [BookingStatus.CHECKED_IN.value, 
